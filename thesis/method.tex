...
\todo{Perhaps an illustration with many of these processes next to each other
is better?}
...
get two different types of calculations: one depending on some peripheral unit
of the CPU, for instance the file system or the network unit, and one depending
on the CPU and its memory. The first we call I/O-work, or $\lambda^{io}$, for
it is work not necessarily done by the CPU but by some peripheral I/O unit. The
latter we call CPU-work, or $\lambda^{cpu}$, for it is work entirely performed
by the CPU. The definition then follows that the total work done by the event
handler $\lambda^e_j = \lambda^{io}_j + \lambda^{cpu}_j$ for an event
$\epsilon_j$. It is however not necessarily the case that $\lambda^{io}$ is
disjunct from $\lambda^{cpu}$, they might depend on each other to some extent.
