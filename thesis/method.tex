...
\begin{figure}[h!]
    ...
    \caption{A sequence diagram of the serial event propagation model. Only one
    event $\epsilon_j$ is dispatched and handled at a time and no event
    $\epsilon_j$ can be dispatched before the previous work $\Lambda_{j-1}$ is
    finished.}
...
\begin{figure}[h!]
    ...
    \caption{Workflow of the preemptive event handler. Serial work is done on
    multiple threads.}
...
\begin{figure}[h!]
    ...
    \caption{Workflow of the cooperative event handler. CPU work $\lambda_0$ is
    performed on a single thread, I/O work $\lambda_1$ is however performed
    asynchronously, thus letting event handler return immediately after
    $\lambda_0$ is done.}
...
\begin{table}[h!]
    \caption{Overview of the combination of event propagation models available
    for the dispatcher and event handler.}
