...
\begin{ctable}
    {The parameters enabled for configuration on the test manager and the
    gateway processes.}
    {|l|p{8cm}|}
    ...
    Name                & Description \\
    \hline
    Quantity ($q$)      & Number of devices to simulate. \\
    \hline
    Frequency ($\phi$)  & The frequency in events per seconds for which each
    device creates a new event. \\
    \hline
    Delay ($\delta$)    & The delay in seconds added for each TCP call made on
    the network. \\
    \hline
    Dispatcher ($\kappa_0$) & The dispatcher model used. Either "serial",
    "cooperative" or "preemptive". \\
    \hline
    Event handler ($\kappa_1$)  & The event handler model used. Same options as
    for $\kappa_0$. \\
    \hline
    CPU intensity ($\lambda_0$) & The CPU intensity induced by each event. Must
    be a value between 0 and 1. \\
    \hline
    I/O intensity ($\lambda_1$) & The I/O intensity induced by each event. Must
    be a value between 0 and 1. \\
    \hline
    Threadpool size             & The max number of threads available for the
    gateway. \\
    \hline
    Duration                    & The time the test scenario will run before stopping.
    \\
    \hline
    Log level                   & The log level on the test manager. Can be set
    to "VERBOSE", "DEBUG", "INFO" and "ERROR". \\
    \hline
    Database path               & The filesystem path to the database. \\
    \hline
    Report name                 & The name of the report this test scenario
    belongs to. \\
    \hline

\end{ctable} \label{tab:config_parameters}
\fi
